...
// clustering with a dynamical jet resolution f, for higher powers of transverse
// momentum weights. The code contains N point correlators up to N = 8. In this
// case, we need to add sum of pT^weights of jet constituents to obtain the
// correct description that can be compared to the full calculation.
//
...
//Global variables that can be accessed directly in the main code eec_fast_weight.cc/eec_fast_kt_weight.cc
...
class EEC {
public:
    EEC(fastjet::PseudoJet tree, double jpt, int n, double weight, const double minbin, const int nbins) {
        compute(tree, jpt, n, weight, minbin, nbins);
    }
    
    void compute(const fastjet::PseudoJet& tree, double jpt, int n, double weight, const double minbin, const int nbins) { // Pass by reference
        ...
        if (tree.has_parents(p1, p2)) 
        {
            ...
            const double dist = std::pow(th,2)/std::pow(R,2);
            
            //Resolve as much of each of the branches as needed
            b1 = p1.exclusive_subjets(dist / fac);
            b2 = p2.exclusive_subjets(dist / fac);
            
            std::vector<double> pt_b1_arr, pt_b2_arr;
            
            // Iterate over the size of branch 1
            for (const fastjet::PseudoJet& subjet : b1) {
                // Get the constituents of the subjet
                std::vector<fastjet::PseudoJet> subjet_constituents = subjet.constituents();
                // Sum pT^weights of jet constituents
                double pt_b1 = 0.0;
                for (const fastjet::PseudoJet& constituent : subjet_constituents) {
                    pt_b1 += std::pow(constituent.pt()/jpt, weight);
                }
                pt_b1_arr.push_back(pt_b1);
            }
            
            // Iterate over the size of branch 2
            for (const fastjet::PseudoJet& subjet2 : b2) {
                // Get the constituents of the subjet
                std::vector<fastjet::PseudoJet> subjet_constituents2 = subjet2.constituents();
                // Sum pT^weights of jet constituents
                double pt_b2 = 0.0;
                for (const fastjet::PseudoJet& constituent2 : subjet_constituents2) {
                    pt_b2 += std::pow(constituent2.pt()/jpt, weight);
                }
                pt_b2_arr.push_back(pt_b2);
            }
            ...
            pt_b1_arr.insert(pt_b1_arr.end(), pt_b2_arr.begin(), pt_b2_arr.end());
                ...
                case 2:
                {
                    ...
                    for (i[0] = 0; i[0] < nb1; ++i[0])
                        for (i[1] = std::max(i[0], nb1); i[1] < nb1 + nb2; ++i[1])
                        {
                            ...
                            //Add to histogram, including permutation factor and weight of particles transverse momentum fractions stored in pt_b1_arr, pt_b2_arr 
                            double efac = 1;
                            for (int j = 0; j < npt; ++j)
                                efac *= pt_b1_arr[i[j]];
            ...
            compute(p1, jpt, n, weight, minbin, nbins);
            compute(p2, jpt, n, weight, minbin, nbins);
        } else {
            // Compute the correlator when all transverse momenta weights are kept on the same particle
            res[0] += std::pow((tree.pt() / jpt), weight*n);
        }
    }
};
